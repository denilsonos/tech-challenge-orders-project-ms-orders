import { ItemDAO } from "../../base/dao/item";
import { ItemUseCaseImpl } from "../../core/use-cases/item/item-use-case";
import { DbConnectionImpl } from "../../frameworks/database/db-connection-impl";
import { ItemRepositoryImpl } from "../repositories/item-repository";
import { ItemController } from "./item-controller";
import { ItemCategory } from "./validators/enums/item-category";

jest.mock('typeorm', () => {
    return {
        Entity: () => jest.fn(),
        PrimaryGeneratedColumn: () => jest.fn(),
        Column: () => jest.fn(),
        CreateDateColumn: () => jest.fn(),
        UpdateDateColumn: () => jest.fn(),
        ManyToMany: () => jest.fn(),
        JoinTable: () => jest.fn(),
        OneToOne: () => jest.fn(),
        JoinColumn: () => jest.fn(),
    }
});

jest.mock('../../frameworks/database/db-connection-impl', () => {
    return {
        DbConnectionImpl: jest.fn().mockImplementation(() => {
            return {
                getConnection: jest.fn().mockReturnValue({
                    getRepository: jest.fn().mockReturnValue({
                        findOneBy: jest.fn().mockResolvedValue(null),
                        find: jest.fn(),
                        save: jest.fn(),
                        delete: jest.fn().mockReturnThis(),
                        update: jest.fn().mockReturnThis(),
                    })
                })
            }
        })
    }
});

describe('ItemController', () => {

    let database: DbConnectionImpl;
    let itemController: ItemController;
    let itemUseCase: ItemUseCaseImpl;
    let itemRepository: ItemRepositoryImpl;

    beforeAll(() => {
        database = new DbConnectionImpl();
        itemController = new ItemController(database);
        itemRepository = new ItemRepositoryImpl(database);
        itemUseCase = new ItemUseCaseImpl(itemRepository);
    });

    beforeEach(() => {
        jest.clearAllMocks();
    });

    it('should create item', async () => {
        // Arrange
        const bodyRequest = {
            name: 'X Bacon',
            description: 'A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
            category: ItemCategory.Snack,
            value: 19,
            image: ''
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = 1;
        itemDAO.name = bodyRequest.name
        itemDAO.description = bodyRequest.description;
        itemDAO.category = bodyRequest.category;
        itemDAO.value = bodyRequest.value;
        itemDAO.image = Buffer.from(bodyRequest.image)

        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'save').mockResolvedValue(itemDAO);

        // Act
        const result = await itemController.create(bodyRequest);
        // Assert
        expect(result).toBe(1);
        expect(database.getConnection().getRepository(ItemDAO).save).toHaveBeenCalledTimes(1);
    });

    it('should fail when create item', async () => {
        // Arrange
        const bodyRequest = {
            name: 'X Bacon',
            description: 'A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
            category: ItemCategory.Snack,
            value: 19,
            image: 'image'
        }

        jest.spyOn(itemUseCase, 'create').mockRejectedValue(null)

        // Act
        try {
            await itemController.create(bodyRequest)
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(itemUseCase.create).toHaveBeenCalledTimes(0);
        }

    });

    it('should delete item', async () => {
        // Arrange
        const id = 1;
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'delete');
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'findOneBy').mockResolvedValue(new ItemDAO());
        // Act
        await itemController.delete({ id });
        // Assert
        expect(database.getConnection().getRepository(ItemDAO).delete).toHaveBeenCalledTimes(1);
    });

    it('should fail when delete item', async () => {
        // Arrange
        const id = 1;
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'findOneBy').mockResolvedValue(null);
        // Act
        try {
            await itemController.delete({ identifier: id });
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(database.getConnection().getRepository(ItemDAO).delete).toHaveBeenCalledTimes(0);
        }
    });

    it('should update item', async () => {
        // Arrange
        const bodyRequest = {
            id: 1,
            name: 'X Bacon',
            description: 'A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
            category: ItemCategory.Snack,
            value: 19,
            image: ''
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = bodyRequest.id;
        itemDAO.name = bodyRequest.name
        itemDAO.description = bodyRequest.description;
        itemDAO.category = bodyRequest.category;
        itemDAO.value = bodyRequest.value;
        itemDAO.image = Buffer.from(bodyRequest.image)
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'findOneBy').mockResolvedValue(itemDAO);
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'update');

        // Act
        await itemController.update(
            {
                id: bodyRequest.id
            },
            bodyRequest
        );
        // Assert
        expect(database.getConnection().getRepository(ItemDAO).update).toHaveBeenCalledTimes(1);
    });

    it('should fail when update item and body is invalid', async () => {
        // Arrange
        const bodyRequest = {
            id: 1,
            name: 'X Bacon',
            description: 'A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
            category: ItemCategory.Snack,
            value: 19,
            image: 'any image'
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = bodyRequest.id;
        itemDAO.name = bodyRequest.name
        itemDAO.description = bodyRequest.description;
        itemDAO.category = bodyRequest.category;
        itemDAO.value = bodyRequest.value;
        itemDAO.image = Buffer.from(bodyRequest.image)
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'update');

        // Act
        try {
            await itemController.update(
                {
                    id: bodyRequest.id
                },
                bodyRequest
            );
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(database.getConnection().getRepository(ItemDAO).update).toHaveBeenCalledTimes(0);
        }
    });

    it('should fail when update item and body is invalid', async () => {
        // Arrange
        const bodyRequest = {
            id: 1,
            name: 'X Bacon',
            description: 'A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
            category: ItemCategory.Snack,
            value: 19,
            image: 'any image'
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = bodyRequest.id;
        itemDAO.name = bodyRequest.name
        itemDAO.description = bodyRequest.description;
        itemDAO.category = bodyRequest.category;
        itemDAO.value = bodyRequest.value;
        itemDAO.image = Buffer.from(bodyRequest.image)
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'update');

        // Act
        try {
            await itemController.update(
                {
                    id: null
                },
                bodyRequest
            );
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(database.getConnection().getRepository(ItemDAO).update).toHaveBeenCalledTimes(0);
        }
    });

    it('should get item by id', async () => {
        // Arrange
        const paramsRequest = {
            id: 1
        }
        
        const itemDAO = new ItemDAO();
        itemDAO.id = 1;
        itemDAO.name = 'X Bacon';
        itemDAO.description ='A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
        itemDAO.category = ItemCategory.Snack,
        itemDAO.value = 19,
        itemDAO.quantity = 1
        itemDAO.image = Buffer.from('')
        
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'findOneBy').mockResolvedValue(itemDAO);
        // Act
        const result = await itemController.getById({ id: paramsRequest.id });
        // Assert
        expect(result).toEqual(itemDAO);
        expect(database.getConnection().getRepository(ItemDAO).findOneBy).toHaveBeenCalledTimes(1);
    });

    it('should fail when get item by id and id is invalid', async () => {
        // Arrange
        const paramsRequest = {
            id: null
        }
        
        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'findOneBy');
        // Act
        try {
            await itemController.getById({ id: paramsRequest.id });
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(database.getConnection().getRepository(ItemDAO).findOneBy).toHaveBeenCalledTimes(0);
        }
    });

    it('should get item by params', async () => {
        // Arrange
        const paramsRequest = {
            category: ItemCategory.Snack
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = 1;
        itemDAO.name = 'X Bacon';
        itemDAO.description ='A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
        itemDAO.category = ItemCategory.Snack,
        itemDAO.value = 19,
        itemDAO.quantity = 1
        itemDAO.image = Buffer.from('')

        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'find').mockResolvedValue([itemDAO]);
        
        // Act
        const result = await itemController.findByParams(paramsRequest);

        // Assert
        expect(result).toEqual([itemDAO]);
        expect(database.getConnection().getRepository(ItemDAO).find).toHaveBeenCalledTimes(1);
    });

    it('should fail when get item by params and params is invalid', async () => {
        // Arrange
        const paramsRequest = {
            category: null
        }

        const itemDAO = new ItemDAO();
        itemDAO.id = 1;
        itemDAO.name = 'X Bacon';
        itemDAO.description ='A hamburger is a sandwich made of a grilled or fried patty of ground meat, usually beef, served in a bun or roll.',
        itemDAO.category = ItemCategory.Snack,
        itemDAO.value = 19,
        itemDAO.quantity = 1
        itemDAO.image = Buffer.from('')

        jest.spyOn(database.getConnection().getRepository(ItemDAO), 'find').mockResolvedValue([itemDAO]);
        
        // Act
        try {
            await itemController.findByParams(paramsRequest);
        } catch (error) {
            // Assert
            expect((error as any).message).toEqual("Validation error!");
            expect(database.getConnection().getRepository(ItemDAO).find).toHaveBeenCalledTimes(0);        
        }
    });
})